...
@interface WRecordCircleView ()
@property (nonatomic, strong)NSString *oldStep;
...
@implementation WRecordCircleView
...
- (void)setLabel {
    ...
    self.stepLabel.font = [UIFont fontWithName:@"Helvetica" size:kScreenH / 16.f];
    ...
    [titleLable mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self.stepLabel.mas_top).offset(-5);
    ...
    [self.totalLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.stepLabel.mas_bottom).offset(5);
...
- (void)drawCircle {
    ...
    self.shapeLayer.strokeColor = [UIColor mainColor].CGColor;
...
- (void)circleAnimation:(BOOL)animation nowStep:(NSUInteger)nowStep {
    if (animation == NO) {
        return;
    }
...
- (void)setNowStep:(NSString *)nowStep {
    ...
    [self circleAnimation:YES nowStep:[nowStep integerValue]];
}
