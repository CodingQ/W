...
- (void)createDataX {
    ...
    for (int index = 0; index < number; index++) {
        ...
        if (self.unit == WUnitDay || self.unit == WUnitWeak) {
...
- (void)createDataY {
    ...
    [self drawDashLine:count height:viewHeight];
...
- (void)drawDashLine:(int)count height:(CGFloat)viewHeight {
    if (!self.hasDashLine) {
        return ;
    }
...
- (void)p_showNoMessageView {
    ...
    [self.noMessageLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(self.gradientView.mas_centerY);
        make.width.mas_equalTo(kScreenW / 2.0);
        make.height.mas_equalTo(self.gradientView.frame.size.height / 2.0);
    }];
}
